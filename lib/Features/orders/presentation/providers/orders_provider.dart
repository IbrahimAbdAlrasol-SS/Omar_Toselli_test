...
import 'package:Tosell/features/orders/data/services/orders_service.dart';
...
class OrdersNotifier extends _$OrdersNotifier {
  ...
  Future<(Order? order, String? error)> addOrder(AddOrderForm form) async {
    developer.log('🚀 OrdersNotifier.addOrder() - بدء عملية إضافة طلب',
        name: 'OrdersProvider');

    try {
      developer.log('⏳ تعيين حالة التحميل في OrdersNotifier...',
          name: 'OrdersProvider');
      state = const AsyncValue.loading();

      developer.log('🔄 استدعاء خدمة الطلبات من OrdersNotifier...',
          name: 'OrdersProvider');
      var result = await _service.addOrder(orderForm: form);

      developer.log('📡 نتيجة الخدمة في OrdersNotifier:',
          name: 'OrdersProvider');
      developer.log('  - Order Created: ${result.$1 != null}',
          name: 'OrdersProvider');
      developer.log('  - Order Code: ${result.$1?.code ?? "N/A"}',
          name: 'OrdersProvider');
      developer.log('  - Error Message: ${result.$2 ?? "N/A"}',
          name: 'OrdersProvider');

      if (result.$1 != null) {
        developer.log('✅ تم إنشاء الطلب بنجاح، تحديث القائمة...',
            name: 'OrdersProvider');
        await refresh();
        return (result.$1, null);
      } else {
        developer.log('❌ فشل في إنشاء الطلب في OrdersNotifier: ${result.$2}',
            name: 'OrdersProvider');
        return (null, result.$2);
      }
    } catch (e) {
      developer.log('💥 خطأ في OrdersNotifier.addOrder(): $e',
          name: 'OrdersProvider');
  ...
  Future<void> search(String searchTerm) async {
    try {
      ...
      final queryParams =
          searchTerm.isNotEmpty ? {'code': searchTerm} : <String, dynamic>{};
      final result = await getAll(page: 1, queryParams: queryParams);
      state = AsyncValue.data(result.data ?? []);
    } catch (e) {
      state = AsyncValue.error(e, StackTrace.current);
