...
class OrderCommandsNotifier extends _$OrderCommandsNotifier {
  ...
  Future<(Order? order, String? error)> addOrder( AddOrderForm form) async {
    developer.log('🚀 OrderCommandsNotifier.addOrder() - بدء عملية إضافة طلب جديد', name: 'OrderCommands');
    developer.log('📝 Form Data Received:', name: 'OrderCommands');
    developer.log('  - Customer Name: ${form.customerName}', name: 'OrderCommands');
    developer.log('  - Customer Phone: ${form.customerPhoneNumber}', name: 'OrderCommands');
    developer.log('  - Delivery Zone ID: ${form.deliveryZoneId}', name: 'OrderCommands');
    developer.log('  - Pickup Zone ID: ${form.pickupZoneId}', name: 'OrderCommands');
    ...
    try {
      ...
      developer.log('  - Order: ${result.$1?.code ?? "null"}', name: 'OrderCommands');
      ...
      if (result.$1 == null) {
        developer.log('❌ فشل في إنشاء الطلب: ${result.$2}', name: 'OrderCommands');
        return (null, result.$2);
      }
      
      developer.log('✅ تم إنشاء الطلب بنجاح - كود الطلب: ${result.$1!.code}', name: 'OrderCommands');
      return (result.$1, null); // success result
    } catch (e) {
      developer.log('💥 خطأ في OrderCommandsNotifier.addOrder(): $e', name: 'OrderCommands');
