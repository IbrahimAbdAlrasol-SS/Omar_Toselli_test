...
class OrdersService {
  ...
  Future<(Order? order, String? error)> addOrder(
    ...
    developer.log('🌐 OrdersService.addOrder() - بدء استدعاء API لإضافة طلب', name: 'OrdersService');
    ...
    try {
      ...
      developer.log('  - Endpoint: ${OrderEndpoints.order}', name: 'OrdersService');
      ...
      developer.log('  - Single Data: ${result.singleData?.code ?? "null"}', name: 'OrdersService');
      developer.log('  - Message: ${result.message ?? "null"}', name: 'OrdersService');
      developer.log('  - Success: ${result.singleData != null}', name: 'OrdersService');
      
      if (result.singleData == null) {
        developer.log('❌ فشل في إنشاء الطلب في الخدمة: ${result.message}', name: 'OrdersService');
        return (null, result.message);
      }
      
      developer.log('✅ تم إنشاء الطلب بنجاح في الخدمة - كود الطلب: ${result.singleData!.code}', name: 'OrdersService');
      return (result.singleData, null);
    } catch (e) {
      developer.log('💥 خطأ في OrdersService.addOrder(): $e', name: 'OrdersService');
      rethrow;
    }
  }
}
